...
class ReleaseManager:
    ...
    def bump_version(self, current: str, bump_type: str) -> str:
        ...
        version = semver.VersionInfo.parse(
            current[1:] if current.startswith("v") else current
        )
    ...
    def run_tests(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                high_issues = [
                    i
                    for i in bandit_results.get("results", [])
                ...
                if high_issues:
                    ...
                    for issue in high_issues[:3]:  # Show first 3
                        print(
                            f"  - {issue['issue_text']} at {issue['filename']}:{issue['line_number']}"
                        )
                else:
    ...
    def run_benchmarks(self) -> bool:
        ...
        try:
            ...
            small_file_time = results.get("small_file", {}).get(
                "processing_time", float("inf")
            )
            medium_file_memory = results.get("medium_file", {}).get(
                "memory_peak_mb", float("inf")
            )

            if small_file_time > 5.0:
                print(
                    f"⚠️  Small file processing slow: {small_file_time:.2f}s (threshold: 5.0s)"
                )
            else:
            ...
            if medium_file_memory > 500:
                print(
                    f"⚠️  High memory usage: {medium_file_memory:.1f}MB (threshold: 500MB)"
                )
            else:
    ...
    def validate_docker_build(self) -> bool:
        ...
        try:
            ...
            # Clean up
            subprocess.run(
                ["docker", "rmi", "customer-snapshot:test"], capture_output=True
            )
    ...
    def perform_release(
        ...
        if not skip_pr:
            ...
            if branch:
                ...
                print("2. Get PR reviewed and approved")
                print("3. Merge PR")
                print("4. Run: git checkout main && git pull")
                print(f"5. Run: git tag -a {version} -m 'Release {version}'")
                print(f"6. Run: git push origin {version}")
                print("7. GitHub Actions will handle the rest!")
        else:
            print("\n✅ Release preparation complete!")
            print("Next steps:")
            ...
            print("4. GitHub Actions will handle the rest!")
...
def main():
    ...
    parser.add_argument("--version", help="Explicit version (overrides bump type)")
    parser.add_argument("--skip-tests", action="store_true", help="Skip running tests")
    parser.add_argument(
        "--skip-pr", action="store_true", help="Skip creating pull request"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    success = manager.perform_release(
        new_version, args.bump_type, skip_tests=args.skip_tests, skip_pr=args.skip_pr
    )
