...
class MonitoringDashboard:
    ...
    def display_dashboard(self):
        ...
        try:
            ...
            print(
                f"🔧 Monitoring Available: {'✅ Yes' if status['monitoring_available'] else '❌ Limited'}"
            )
            print()
            ...
            print(
                f"🌡️  OVERALL STATUS: {status_emoji.get(overall_status, '❓')} {overall_status.upper()}"
            )
            print()
    ...
    def _display_system_section(self, system_data: dict[str, Any]):
        ...
        print(
            f"   📊 Prometheus: {'✅ Running' if prometheus_running else '❌ Stopped' if prometheus_enabled else '⚪ Disabled'}"
        )
    ...
    def _display_alerting_section(self, alerting_data: dict[str, Any]):
        ...
        print(
            f"   🔄 Processing: {'✅ Active' if processing_enabled else '❌ Stopped'}"
        )
        ...
        if active_notifiers:
            ...
            print("   📢 Active Channels: None configured")
    ...
    def _display_performance_section(self, status_data: dict[str, Any]):
        ...
        try:
            ...
            print(
                f"   🖥️  CPU Usage: {self._format_metric_with_status(cpu_percent, 70, 90)}%"
            )
            print(
                f"   🧠 Memory Usage: {self._format_metric_with_status(memory.percent, 80, 95)}%"
            )
            print(
                f"   💾 Disk Usage: {self._format_metric_with_status((disk.used / disk.total) * 100, 80, 95)}%"
            )
            ...
            if hasattr(os, "getloadavg"):
                try:
                    ...
                    print(
                        f"   ⚡ Load Average: {self._format_metric_with_status(load_percent, 100, 200)}% (1m)"
                    )
                except OSError:
                    pass
    ...
    def _format_metric_with_status(
    ...
    def interactive_mode(self):
        ...
        try:
            ...
            while self.running:
                ...
                if select.select([sys.stdin], [], [], self.refresh_interval) == (
                    [sys.stdin],
                    [],
                    [],
                ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Customer Solution Snapshot Generator Monitoring Dashboard"
    )
    parser.add_argument(
        "--mode",
        choices=["interactive", "status", "health", "report"],
        default="interactive",
        help="Dashboard mode",
    )
    parser.add_argument("--output", help="Output file for reports")
    parser.add_argument(
        "--interval", type=int, default=5, help="Refresh interval for interactive mode"
    )
