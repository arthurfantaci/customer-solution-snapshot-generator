        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            try:
                ...
                if proc.info["cmdline"] and any(
                    self.process_name in arg for arg in proc.info["cmdline"]
                ):
                    return proc
...
class PerformanceMonitor:
    ...
    def _monitor_loop(self):
        ...
        while self.running:
            try:
                ...
                app_alerts = []
                if app_metrics:
                    app_alerts = self.alert_manager.check_application_alerts(
                        app_metrics
                    )
    ...
    def _generate_report(self):
        ...
        if summary:
            ...
            print(
                f"   CPU: {summary['cpu']['current']:.1f}% current, "
                f"{summary['cpu']['average']:.1f}% avg, "
                f"{summary['cpu']['max']:.1f}% peak"
            )
            print(
                f"   Memory: {summary['memory']['current']:.1f}% current, "
