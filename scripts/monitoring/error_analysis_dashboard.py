...
class ErrorAnalysisDashboard:
    ...
    def _display_severity_breakdown(self, stats):
        ...
        for severity in severity_order:
            ...
            if count > 0:
                percentage = (
                    (count / total_by_severity) * 100 if total_by_severity > 0 else 0
                )
    ...
    def _display_category_breakdown(self, stats):
        ...
        sorted_categories = sorted(
            stats.errors_by_category.items(), key=lambda x: x[1], reverse=True
        )
        ...
        for category, count in sorted_categories[:8]:  # Top 8 categories
            if count > 0:
                percentage = (
                    (count / total_by_category) * 100 if total_by_category > 0 else 0
                )
    ...
    def _display_top_errors(self, top_errors):
        ...
        for i, error in enumerate(top_errors[:5], 1):
            ...
            print(
                f"      Last Seen: {self._format_time_ago(datetime.fromisoformat(error['last_seen']))}"
            )
            print()
    ...
    def _display_error_trends(self):
        ...
        try:
            ...
            for trend in trends:
                ...
                if category != "none":
                    print(
                        f"      📂 Top Category: {category.replace('_', ' ').title()}"
                    )
                print()
    ...
    def interactive_mode(self):
        ...
        print(
            "🚀 Starting Customer Solution Snapshot Generator Error Analysis Dashboard"
        )
        print("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Customer Solution Snapshot Generator Error Analysis Dashboard"
    )
    parser.add_argument(
        "--mode",
        choices=["interactive", "report", "stats"],
        default="interactive",
        help="Dashboard mode",
    )
    parser.add_argument("--output", help="Output file for reports")
    parser.add_argument(
        "--hours", type=int, default=24, help="Hours for recent error analysis"
    )
