...
class DeploymentAutomation:
    ...
    def __init__(self, project_root: Optional[Path] = None):
        ...
        self.docker_compose_template = self.project_root / "deployment" / "docker-compose.template.yml"
    ...
    def build_docker_image(self, tag: str = "latest", push: bool = False) -> str:
        ...
        build_cmd = [
            ...
            "--build-arg", f"VCS_REF={self.get_git_commit()}",
    ...
    def create_docker_compose_file(self, environment: str) -> Path:
        ...
        compose_file = self.project_root / "deployment" / "docker" / f"docker-compose.{environment}.yml"
                ...
                'template': {
                    ...
                    'spec': {
                        'containers': [{
                            ...
                            'env': [{'name': k, 'value': v} for k, v in env_config.environment_vars.items()],
                            'resources': {
                                'limits': {
                                    'memory': env_config.memory_limit,
    ...
    def deploy_kubernetes(self, environment: str, namespace: str = "default"):
        ...
        logger.info(f"Deploying to {environment} on Kubernetes (namespace: {namespace})")
    ...
    def rollback_deployment(self, environment: str, deployment_type: str = "docker-compose"):
    ...
    def health_check(self, environment: str, timeout: int = 60) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Container status check: {status} ({'healthy' if is_healthy else 'unhealthy'})")
                return is_healthy
    ...
    def get_deployment_status(self, environment: str) -> Dict[str, Any]:
        ...
        status['health'] = 'healthy' if self.health_check(environment, timeout=10) else 'unhealthy'
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Customer Solution Snapshot Generator Deployment")
    parser.add_argument('command', choices=['build', 'deploy', 'rollback', 'status', 'health'], 
    ...
    parser.add_argument('--build', action='store_true', help='Build image before deployment')
