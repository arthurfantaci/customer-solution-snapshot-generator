from pycaption import WebVTTReader, WebVTTWriter, CaptionNode, CaptionSet, CaptionList
import webvtt
import nltk
...
exclude_words = {"um", "uh", "like", "you know", "I mean", "okay", "yeah", "well", "right", "just", "really", "actually", "basically", "literally", "anyway", "anyways", "anyway", "anyways", "kinda", "sorta", "sort of", "kind of", "a lot", "in fact", "understood", "great", "excellent"}
...
def clean_text(text):
    ...
    for token in doc:
        ...
        if (lower_token not in STOP_WORDS and 
            lower_token not in exclude_words and 
            not token.is_punct):
...
def extract_text(captions):
    ...
    for caption in captions:
        caption_text = ''.join(node.content for node in caption.nodes if hasattr(node, 'content'))
...
def enhance_content(text, min_freq=2, min_length=3):
    ...
    common_expressions = set(["you know", "Excellent", "Great", "Understood", "Okay", "I mean", "kind of", "sort of", "a lot", "in fact", "Understood", "Great", "Okay, ", "Alright, "])
...
def resolve_coreferences(text):
    ...
    # Replace pronouns
    resolved_text = re.sub(r'\b(he|she|it|they|them)\b', replace_pronoun, text, flags=re.IGNORECASE)
