...
class DependencyHealthCheck(BaseHealthCheck):
    ...
    def _check_anthropic_api(self) -> dict[str, Any]:
        ...
        try:
            ...
            client = anthropic.Anthropic(
                api_key=self.config.anthropic_api_key, timeout=10.0
            )
    ...
    def _check_python_environment(self) -> dict[str, Any]:
        ...
        try:
            ...
            required_packages = ["spacy", "nltk", "webvtt-py", "anthropic"]
...
class PerformanceHealthCheck(BaseHealthCheck):
    ...
    def _perform_check(self) -> HealthCheckResult:
        ...
        if processing_metrics["response_time_ms"] > self.response_time_threshold_ms:
            status = HealthStatus.WARNING
            issues.append(
                f"Slow processing: {processing_metrics['response_time_ms']:.0f}ms"
            )
        ...
        if MONITORING_AVAILABLE:
            ...
            if (
                memory_metrics.get("growth_rate_mb", 0)
                > self.memory_growth_threshold_mb
            ):
                status = HealthStatus.WARNING
                issues.append(
                    f"High memory growth: {memory_metrics['growth_rate_mb']:.1f}MB"
                )
        ...
        return {
            ...
            "summary": {
                ...
                "average_response_time_ms": sum(
                    r.response_time_ms for r in results.values()
                )
                / len(results)
                if results
                else 0,
            ...
            try:
                ...
                for check_name, check_result in summary["checks"].items():
                    if check_result["status"] in ["warning", "critical"]:
                        logger.warning(
                            f"Health check {check_name}: {check_result['message']}"
                        )
        ...
        for check_name in self.health_checks[0].name if self.health_checks else []:
            ...
            if check_statuses:
                trends[check_name] = {
                    ...
                    "latest_status": check_statuses[-1].value
                    if check_statuses
                    else "unknown",
                }
