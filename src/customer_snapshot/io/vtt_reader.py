...
from customer_snapshot.utils.memory_optimizer import StreamingVTTReader, memory_profile
...
class VTTReader:
    ...
    def read_vtt(self, file_path: Union[str, Path]) -> str:
        ...
        try:
            ...
            if file_size > 10:  # Use streaming for large files
                logger.info(
                    f"Using streaming reader for large file ({file_size:.1f} MB)"
                )
    ...
    def read_vtt_streaming(self, file_path: Union[str, Path]) -> str:
        ...
        try:
            ...
            logger.info(
                f"Successfully read VTT file with {subtitle_count} captions (streaming)"
            )
            logger.debug(f"Extracted {len(full_text)} characters of text")
    ...
    def get_vtt_metadata(self, file_path: Union[str, Path]) -> dict:
        ...
        try:
            ...
            metadata = {
                ...
                "speaker_names": sorted(speakers) if speakers else [],
