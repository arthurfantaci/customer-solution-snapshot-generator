...
from ..utils.validators import safe_file_write, sanitize_filename
...
class OutputWriter:
    ...
    def _format_html(self, content: str, metadata: Dict[str, Any] = None) -> str:
        """
        ...
        if metadata:
            metadata_items = []
            metadata_items.append(f"<li><strong>Generated:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</li>")
            
            for key, value in metadata.items():
                if key != "generated_at":
                    formatted_key = key.replace('_', ' ').title()
                    metadata_items.append(f"<li><strong>{formatted_key}:</strong> {value}</li>")
...
<body>
    ...
    def _enhance_markdown_structure(self, content: str) -> str:
        ...
        for line in lines:
            ...
            if any(keyword in line.lower() for keyword in ['entities identified', 'topics discussed', 'summary', 'overview']):
