    ...
    try:
        ...
        if validate_only:
            ...
            if is_valid:
                ...
                click.echo(f"   Duration: {metadata.get('duration_seconds', 'Unknown')} seconds")
                click.echo(f"   Speakers: {metadata.get('estimated_speakers', 'Unknown')}")
            else:
...
def config_info(ctx: click.Context) -> None:
    ...
    try:
        ...
        click.echo(f"   Max file size: {config_dict.get('max_file_size') // (1024*1024)}MB")
...
@cli.command()
@click.option('--output', '-o', type=click.Path(path_type=Path), default='test_output.md',
